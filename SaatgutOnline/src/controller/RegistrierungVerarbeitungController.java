...
/**
 * <p>
 * Die Klasse <code>RegistrierungVerarbeitungController</code> verarbeitet die Daten aus dem Registrierungsformular
 * <code>RegistrierungView</code> mit der Methode <code>validiereKundendaten(request)</code>. Je nach Ergebnis der
 * Auswertung wird auf entsprechende Seiten weitergeleitet.
 * </p>
 * 
...
public class RegistrierungVerarbeitungController
{
	...
	/**
	 * <p>
	 * Konstruktor der Klasse<code>RegistrierungVerarbeitungController</code>. Mit
	 * <code>validiereKundendaten(request)</code> werden die Formulareingaben ueberprueft. Die Pruefung prueft auf
	 * nicht null (Pflichtfelder) und weiter auf anderer Eigenschaften, je nach Formularfeld.
	 * <code>RegistrierungView</code> mit der Methode <code>validiereKundendaten(request)</code>. Bei bestandener
	 * Pruefung wird ein Kundenobjekt in die Datenbank gespeichert, ein Hash aus dem Passwort erstellt und dieses
	 * ebenfalls in die Datenbank gespeichert. Weiters wird eine uuid erzeugt und per Email eine
	 * Registrierungsaufforderung an die E-Mail-Adresse des Kunden gesendet.
	 * </p>
	 * 
	 * @author Christof Weigandt
	 * @version 1.0
	...
	private boolean validiereKundendaten(HttpServletRequest request)
	{
		...
		if (request.getParameter("Emailadresse") != null)
		{
			...
			if (request.getParameter("Emailadresse").length() > 4
					&& request.getParameter("Emailadresse").length() < 70)
			{
