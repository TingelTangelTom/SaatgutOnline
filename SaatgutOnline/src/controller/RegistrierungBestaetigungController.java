 ...
 * <code>RegistrierungBestaetigungView</code> und ruft die passende
 * Ausgabemethode derselben auf.
 * </p>
 * 
...
public class RegistrierungBestaetigungController {
	 ...
	 */
	public RegistrierungBestaetigungController(HttpServletRequest request,
			HttpServletResponse response) {
		this.session = request.getSession();
		this.registrierungBestaetigungView = new RegistrierungBestaetigungView(
				request, response);
		...
		if (request.getParameter("uuid") != null) {
			...
			String query = "SELECT * FROM "
					+ KonfigurationController.getDbName()
			...
			if (count == 1) {
				this.registrierungBestaetigungView
						.outRegistrierungBestaetigungView();
			} else {
