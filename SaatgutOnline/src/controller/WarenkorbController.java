...
/**
 * <p>
 * Die Klasse <code>WarenkorbController</code> stellt Kontrollstrukturen zur Darstellung und Organisation des
...
public class WarenkorbController
{
	 ...
	 * </p>
	 * <p>
	 * Ruft die Methode <code>warenkorbAusSessionHolen()</code> auf. </br>Die Methode holt einen bestehenden
	 * Warenkorb aus der <code>HttpSession</code> und legt ihn in der Variable <i>warenkorb</i> ab. Falls kein
	 * Warenkorb in der Session liegt, wird er erzeugt.
	 * </p>
	 * <p>
	 * Ruft die Methode <code>warenkorbAktualiseren()</code> auf. </br>Die Methode stellt die Funktionalitaet fuer
	 * den Warenkorb zur Verfuegung: </br>- Produkt hinzufuegen </br>- Produkt entfernen </br>- Menge aendern
	 * </p>
	 * <p>
	 * Der aktuelle Warenkorb wird in die Session geschrieben.
	 * </p>
	 * 
	 * @param request
	...
	/**
	 * <p>
	 * Formatiert durch den situationsbedingten Aufruf von Methoden der Klasse <code>WarenkorbView</code> die
	 * Darstellung des Warenkorbs und gibt diese aus.
	 * </p>
	 * 
	...
	public void warenkorbAnzeigen()
	{
		...
		if (!this.warenkorb.isEmpty())
		{
			...
			while (produkte.hasMoreElements())
			{
				...
				// die beiden nachfolgenden Zeilen nutzen Code von Simon Ankele
				String einzelpreisFormatiert = htmlAusgabe.outPreisformat(anzuzeigendesProduktModel
						.getPreisBrutto());
		...
		// die nachfolgende Zeile nutzt Code von Simon Ankele
	...
	private void warenkorbAktualisieren()
	{
		...
		{
			int hinzugefuegteMenge = 0;
			boolean istInteger = true;
			if (this.request.getParameter("menge") != null)
			{
				try
				{
					hinzugefuegteMenge = Integer.parseInt(this.request.getParameter("menge"));
				}
				catch (NumberFormatException e)
				{
					istInteger = false;
				}
				if (istInteger && (hinzugefuegteMenge > 0))
				{
					...
					{
						this.warenkorb.put(produktModelAusDatenbank,
								Integer.parseInt(this.request.getParameter("menge")));
					}
				}
			}
		}
		else
		{
			...
			{
				/*
				 * Da pro Aufruf die Menge mehrere Produkte aktualisiert werden können, gibt es moeglicherweise
				 * mehrere Parameter des Namens "menge". Um die Menge dennoch eindeutig einem Produkt zuordenen zu
				 * koennen, wird bei der Erzeugung des Parameters (per Unterstrich getrennt) die Produkt-ID
				 * angehaengt. Der Ubergabeparameter "menge" ist wie folgt aufgebaut: menge_<produktId>=<neueMenge>
				 * Um nun die Produkt-ID als Wert zu bekommen, mit dem weiter gearbeitet werden kann, wird der
				 * Parameter am Unterstrich gesplittet und die Bestandteile "menge" und <produktId> im StringArray
				 * 'splittedParameter' abgelegt. Die Produkt-ID ist dann als Integer verfuegbar per:
				...
				while (parameters.hasMoreElements())
				{
					parameter = parameters.nextElement();
					/*
					 * Auch hier wird wieder der Parameter bei der Erzeugung um die Produkt-ID erweitert und dann
					 * am Unterstrich gesplittet. (siehe oben)
					 */
